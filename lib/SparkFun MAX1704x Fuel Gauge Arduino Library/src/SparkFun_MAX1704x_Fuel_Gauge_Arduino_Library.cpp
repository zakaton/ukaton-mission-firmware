...
bool SFE_MAX1704X::begin(uint8_t deviceAddress, TwoWire &wirePort)
{
...
bool SFE_MAX1704X::isConnected(void)
{
  ...
  while ((success == false) && (retries > 0))
  {
    ...
    if (_i2cPort->requestFrom(_deviceAddress, 2) == 2) // Attempt to read the version (2 bytes)
    {
...
uint16_t SFE_MAX1704X::read16(uint8_t address)
{
  ...
  while ((success == false) && (retries > 0))
  {
    ...
    if (_i2cPort->requestFrom(_deviceAddress, 2) == 2)
    {
      uint8_t msb = _i2cPort->read();
      uint8_t lsb = _i2cPort->read();
