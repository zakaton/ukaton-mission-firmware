...
bool BNO080::isInterruptOn(void)
{
...
uint16_t BNO080::parseCommandReport(void)
{
	...
	else if (shtpData[1] == SENSOR_REPORTID_SIGNIFICANT_MOTION)
	{
		if (_printDebug == true)
		{
			_debugPort->println(F("SIGNIFICANT MOTION"));
		}
		lastTimeSignificantlyMoved = millis();
		didEnableSignificantMotion = false;
		if (autoEnableSignificantMotion)
		{
			enableSignificantMotionDetector();
		}
	}
	else if (shtpData[0] == SHTP_REPORT_PRODUCT_ID_RESPONSE)
	{
		if (_printDebug == true)
		{
			_debugPort->print(F("Reset reason: "));
			_debugPort->print(shtpData[1], HEX);
...
uint16_t BNO080::parseInputReport(void)
{
	...
	else if (shtpData[5] == SENSOR_REPORTID_STABILITY_DETECTOR)
	{
		lastTimeStabilityDetectorTriggered = millis();
		// Serial.printf("stability detector triggered %u\n", lastTimeStabilityDetectorTriggered);
	}
	...
	else if (shtpData[5] == SENSOR_REPORTID_SHAKE_DETECTOR)
	{
		if (_printDebug == true)
		{
			_debugPort->println(F("SHAKE!"));
		}
	}
	else
	{
...
uint8_t BNO080::requestProductId()
{
	shtpData[0] = SHTP_REPORT_PRODUCT_ID_REQUEST; // Request the product ID and reset info
	shtpData[1] = 0;															// Reserved
...
uint32_t BNO080::floatToQ(float floatingPointValue, uint8_t qPoint)
{
	floatingPointValue *= pow(2, qPoint);
	uint32_t fixedPointValue = floatingPointValue;
...
// FIX - how to properly configure detector?
// https://cdn.sparkfun.com/assets/4/d/9/3/8/SH-2-Reference-Manual-v1.2.pdf
// https://github.com/adafruit/Adafruit_BNO08x/blob/41dc7193fa16c66d7e05c28f0e22cb6063b54d19/src/sh2_SensorValue.h
// Sends the packet to enable the Stability Detector
void BNO080::enableStabilityDetector(uint16_t timeBetweenReports)
{
	setFeatureCommand(SENSOR_REPORTID_STABILITY_DETECTOR, timeBetweenReports);
}

// https://www.ceva-dsp.com/wp-content/uploads/2019/10/BNO080_085-Datasheet.pdf
// https://cdn.sparkfun.com/assets/4/d/9/3/8/SH-2-Reference-Manual-v1.2.pdf
void BNO080::enableSignificantMotionDetector()
{
	if (didEnableSignificantMotion)
	{
		return;
	}

	uint32_t specificConfig = 0;
	specificConfig = (uint32_t)floatToQ(2.0, 24);
	specificConfig += 1 << 24;
	setFeatureCommand(SENSOR_REPORTID_SIGNIFICANT_MOTION, 1000);

	didEnableSignificantMotion = true;
}
void BNO080::disableSignificantMotionDetector()
{
	setFeatureCommand(SENSOR_REPORTID_SIGNIFICANT_MOTION, 0);
	autoEnableSignificantMotion = false;
}
...
{
	_i2cPort->requestFrom((uint8_t)_deviceAddress, requestBytes);
	if (waitForI2C() == false)
		return (false); // Error

	auto readI2CResponse = readI2C();

	if (shtpHeader[2] == CHANNEL_EXECUTABLE && shtpData[0] == EXECUTABLE_RESET_COMPLETE)
	{
		if (_printDebug)
		{
			_debugPort->println("bno reset");
		}
		_hasReset = true;
	}

	return readI2CResponse;
}
uint8_t BNO080::readI2C(void)
{
	// _debugPort->printf("readI2C: %u ", syncCom); _debugPort->flush();
	switch (syncCom)
	{
	case 0:
		byteToRead = _i2cPort->available();
		// _debugPort->printf(" avail: %u\n", byteToRead); _debugPort->flush();
		if (byteToRead >= requestBytes)
		{
			// _debugPort->println(requestBytes);
			for (uint8_t k = 0; k < byteToRead; k++)
			{
				shtpRawData[k] = _i2cPort->read();
			}
			printPacket(requestBytes);
			remainingBytes = getRemainingBytes(shtpRawData[1], shtpRawData[0]);
			// _debugPort->println(remainingBytes);
			if (remainingBytes < (I2C_BUFFER_LENGTH - BNO080_HEADER_LENGTH))
			{
			...
			{
				requestBytes = I2C_BUFFER_LENGTH;
				syncCom = 1;
			}
		}
		if (syncCom == 0)
	...
	case 1:
		...
		if (byteToRead >= requestBytes)
		{
			...
			printPacket(requestBytes);
			remainingBytes -= (I2C_BUFFER_LENGTH - BNO080_HEADER_LENGTH);
			if (remainingBytes < (I2C_BUFFER_LENGTH - BNO080_HEADER_LENGTH))
			{
				requestBytes = remainingBytes;
				syncCom = 2;
	...
	case 2:
		byteToRead = _i2cPort->available();
		// _debugPort->printf(" avail: %u\n", byteToRead); _debugPort->flush();
		if (_i2cPort->available() >= byteToRead)
		{
			for (uint8_t k = 0; k < byteToRead; k++)
			{
				shtpRawData[k] = _i2cPort->read();
			...
			syncCom = 0;
			printPacket(requestBytes);
			requestBytes = BNO080_HEADER_LENGTH;
			if (flagStartup == 0)
			{
				flagStartup = 1;
				if (_printDebug == true)
				{
					_debugPort->println("flagStartup = 1");
				}
			}
		}
		return true;
...
uint16_t BNO080::getRemainingBytes(uint8_t MSB, uint8_t LSB)
{
...
boolean BNO080::getData(uint16_t bytesRemaining)
{
	...
	while (bytesRemaining > 0)
	{
		...
		auto _waitForI2C = waitForI2C();
		if (_waitForI2C == false)
			return (0); // Error
		...
		for (uint8_t x = 0; x < numberOfBytesToRead; x++)
		{
			...
			if (dataSpot < MAX_PACKET_SIZE)
			{
				...
				if (_printDebug == true)
				{
					_debugPort->print(shtpData[dataSpot - 1], HEX);
					_debugPort->print(",");
				}
			}
			else
...
void BNO080::printPacket(uint8_t printBytes)
{
	if (_printDebug)
	{
		// _debugPort->printf("printPacketI2C(%u)\n", printBytes);
		_debugPort->flush();
		for (uint8_t j = 0; j < printBytes; j++)
		{
			_debugPort->print(shtpRawData[j], HEX);
			_debugPort->print(",");
		}
		_debugPort->println();
	}
...
unsigned long BNO080::getLastTimeSignificantlyMoved()
{
	return lastTimeSignificantlyMoved;
}
unsigned long BNO080::getLastTimeStabilityDetectorTriggered()
{
	return lastTimeStabilityDetectorTriggered;
}

void BNO080::setAutoEnableSignificantMotionDetector(bool autoEnable)
{
	autoEnableSignificantMotion = autoEnable;
	if (autoEnableSignificantMotion)
	{
		if (!didEnableSignificantMotion)
		{
